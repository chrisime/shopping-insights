...
from .rewe_bonus_extractor import extract_bonus_amounts_from_text
from .rewe_info_extractor import extract_rewe_receipt_info
from .rewe_items_extractor import extract_rewe_receipt_items
from .rewe_payment_extractor import extract_payment_methods_from_lines


def parse_rewe_ticket(pdf_path: str | Path) -> ReceiptData:
...
def _build_receipt_data(path: Path, text: str, lines: List[str]) -> ReceiptData:
    ...
    total_price = _calculate_total_price(
        metadata.get("total_price"), rewe_bonus_saved_amount
    )
    discount = (
        round(items_result.saved_amount, 2) if items_result.saved_amount > 0 else None
    )
    saved_deposit = (
        round(items_result.saved_deposit, 2) if items_result.saved_deposit > 0 else None
    )
    rewe_bonus_discount = (
        round(rewe_bonus_saved_amount, 2) if rewe_bonus_saved_amount >= 0 else 0.0
    )
    ...
    return build_receipt_schema(
        ...
        bon_number=metadata.get("bon_number"),
