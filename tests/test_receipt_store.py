...
from storage import WRITE_BACKENDS, create_receipt_store
...
class ReceiptStoreTests(unittest.TestCase):
    def test_storage_module_exports_available_write_backends(self):
        self.assertEqual(WRITE_BACKENDS, ("json",))
    ...
    def test_json_receipt_store_load_snapshot_returns_ids_receipts_and_target_path(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            snapshot = JsonReceiptStore().load_receipts_snapshot(
                retailer="rewe",
                file_path=str(file_path),
            )

        self.assertEqual(snapshot.receipts_file, str(file_path))
        self.assertEqual(snapshot.existing_ids, {"a", "legacy-b"})
    ...
    def test_shared_receipt_schema_normalizes_lidl_defaults_and_quantities(self):
        payload = {
