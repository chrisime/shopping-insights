...
class ReceiptStoreTests(unittest.TestCase):
    ...
    def test_json_receipt_store_fetch_receipt_states_reads_source_and_payload_hashes(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            file_path = Path(tmp_dir) / "receipts.json"
            file_path.write_text(
                simplejson.dumps(
            ...
            store = JsonReceiptStore()
            states = store.find_receipt_states(
                retailer="rewe",
                file_path=str(file_path),
            )

        self.assertEqual(states["a"].source_hash, "source-a")
        self.assertEqual(states["a"].payload_hash, "payload-a")
        self.assertIsNone(states["legacy-b"].source_hash)
        self.assertIsNone(states["legacy-b"].payload_hash)
    ...
    def test_sqlite_receipt_store_fetch_receipt_states_returns_source_and_payload_hashes(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            db_path = Path(tmp_dir) / "receipts.sqlite"
            store = SqliteReceiptStore()
            store.persist_receipts(
                [
                    {
                        ...
                        "store": "lidl",
                        "total_price": 19.23,
                        "source_hash": "source-lidl-1",
                        "payload_hash": "payload-lidl-1",
                    }
                ],
                retailer="lidl",
                file_path=str(db_path),
            ...
            states = store.find_receipt_states(retailer="lidl", file_path=str(db_path))

        self.assertEqual(states["23004426420240828113520"].source_hash, "source-lidl-1")
        self.assertEqual(states["23004426420240828113520"].payload_hash, "payload-lidl-1")
    ...
    def test_sort_receipts_by_date_orders_supported_retailer_and_normalized_formats_descending(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            file_path = Path(tmp_dir) / "receipts.json"
            file_path.write_text(
                simplejson.dumps(
    ...
    def test_sqlite_receipt_store_persists_purchase_date_in_iso_format(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with sqlite3.connect(db_path) as connection:
                persisted_purchase_date = connection.execute(
    ...
    def test_sqlite_receipt_store_persist_receipts_reuses_provided_payload_and_source_hashes(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            db_path = Path(tmp_dir) / "receipts.sqlite"
            store = SqliteReceiptStore()
            result = store.persist_receipts(
